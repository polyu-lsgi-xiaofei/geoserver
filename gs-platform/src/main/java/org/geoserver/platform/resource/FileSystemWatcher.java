...
public class FileSystemWatcher implements ResourceNotificationDispatcher, DisposableBean {
	...
	static class Delta {
		...
		public Delta(File context, Kind kind, List<File> created, List<File> removed, List<File> modified) {
			this.context = context;
		...
		public String toString() {
			return "Delta [context=" + context + ", created=" + created + ", removed=" + removed + ", modified="
					+ modified + "]";
		}
	...
	/**
	 * Record of a ResourceListener that wishes to be notified of changes to a
	 * path.
	 */
	private class Watch implements Comparable<Watch> {
		...
		public Delta changed(long now) {
			...
			if (file.isDirectory()) {
				...
				review.removeAll(created); // no need to check these they are
											// new
	...
	/**
	 * Note we have a single runnable here to review all outstanding Watch
	 * instances. The focus is on using minimal system resources while we wait
	 * for Java 7 WatchService (to be more efficient).
	 */
		...
		public void run() {
			...
			for (Watch watch : watchers) {
				...
				if (delta != null) {
					...
					List<ResourceNotification.Event> events = ResourceNotification.delta(watch.file, delta.created,
							delta.removed, delta.modified);

					ResourceNotification notify = new ResourceNotification(watch.getPath(), delta.kind, watch.last,
							events);
	...
	/**
	 * Package visibility to allow test cases to set a shorter delay for
	 * testing.
	 * 
	 * @param delay
	 * @param unit
