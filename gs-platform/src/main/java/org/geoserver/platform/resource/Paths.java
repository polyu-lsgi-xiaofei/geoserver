 ...
 * <p>
 * This utility class is primarily aimed at implementations of ResourceStore and
 * may be helpful when writing test cases. These methods are suitable for static
 * import.
 * <p>
 * Resource paths are consistent with file URLs. The base location is
 * represented with "", relative paths are not supported.
 * 
 * @author XIAO Fei
 */
public class Paths {
	 ...
	 * 
	 * @param strictPath
	 *            whether problematic characters are an error
	 * @param path
	 *            Items defining a Path
	 * @return path Path used to identify a Resource
	 */
	 ...
	 * <ul>
	 * <li>star
	 * <li>colon - potential conflict with xml prefix separator and workspace
	 * style separator
	 * <li>comma
	 * <li>single quote
	 * <li>ampersand
	...
	/**
	 * Set of invalid resource names (currently used to quickly identify
	 * relative paths).
	 */
	static final Set<String> INVALID = new HashSet<String>(Arrays.asList(new String[] { "..", "." }));

	/**
	 * Internal method used to convert a list of names to a normal Resource
	 * path.
	 * 
	 * @param strictPath
	 *            whether problematic characters are an error
	 * @param names
	 *            List of resource names forming a path
	 * @return resource path composed of provided names
	 * @throws IllegalArgumentException
	 *             If names includes any {@link #INVALID} chracters
	 */
	 ...
	 * @return path
	 * @throws IllegalArgumentException
	 *             If path fails {@link #VALID} check
	 */
	...
	/**
	 * Convert to file to resource path, allows for relative references (but is
	 * limited to content within the provided base directory).
	 * 
	 * 
	 * @param base
	 *            directory location
	 * @param folder
	 *            context for relative path (may be "." or null for base
	 *            directory)
	 * @param fileLocation
	 *            File path (using {@link File#separator}) allowing for relative
	 *            references
	 * @return relative path used for Resource lookup
	 */
	 ...
	 * 
	 * This method converts file paths (using {@link File#separator}) to the URL
	 * style paths used for {@link ResourceStore#get(String)}.
	 * 
	 * @param filePath
	 *            File path using {@link File#separator}
	 * @return Resource path suitable for use with
	 *         {@link ResourceStore#get(String)} or null for absolute path
	 */
	...
	/**
	 * Convert a filePath to resource path (starting from the provided path).
	 * Absolute file paths are not supported, and the final resource must still
	 ...
	 * 
	 * @param path
	 *            Initial path used resolve relative reference lookup
	 * @param filename
	 *            File path (using {@link File#separator})
	 * @return Resource path suitable for use with
	 *         {@link ResourceStore#get(String)} or null for absolute path
	 ...
	 * <p>
	 * This method requires the base directory of the ResourceStore. Note
	 * ResourceStore implementations may not create the file until needed. In
	 * the case of an absolute path, base should be null.
	 * 
	 * @param base
	 *            Base directory, often GeoServer Data Directory
	 * @param path
